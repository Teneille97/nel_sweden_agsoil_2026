...
Confidence intervals (CIs) of predicted SOC stocks and $\Delta^{14}\text{C}$ values were much smaller than the standard deviation of observed measurements (Table \ref{tab:sd_pred} and Fig. \ref{fig:fig3}), supporting the value of models that incorporate data from different sites to reveal overall trends.
...
\dataavailability{Data and code associated with this study are available upon reasonable request from the corresponding author.}

\noappendix

\authorcontribution{T.N., C.S., and M.S. conceived and designed the study. M.S. provided field samples and chemical dataset. T.N. conducted data analysis and modeling with guidance from C.S. T.N. drafted the manuscript, and all authors contributed to reviewing and editing.}

\competinginterests{The authors declare that they have no conflict of interest.}

\begin{acknowledgements}
This study was supported by funding from the relevant funding agencies. We thank the staff at the Swedish long-term soil fertility experiments and the Jena MICADAS radiocarbon laboratory.
