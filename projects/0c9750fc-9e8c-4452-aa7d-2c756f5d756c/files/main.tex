...
\begin{abstract}
Soil organic matter (SOM) persistence is a key control on terrestrial carbon cycling, yet it remains unclear whether carbon (C) and nitrogen (N) cycle through SOM at similar rates. Here, we investigated whether the age distributions of C and N diverge in SOM pools in long-term cultivated temperate cropland soils in Sweden. We combined thermal fractionation, elemental analysis, radiocarbon ($\Delta^{14}$C) measurements, and compartmental modeling to quantify differences in C and N persistence. Thermostable SOM fractions contained progressively older C and were systematically enriched in N compared to bulk soil, with C:N ratios declining significantly across the age continuum relative to bulk soil. This empirical divergence demonstrates that long-term C and N persistence are decoupled in agricultural soils, likely driven by selective stabilization of N-containing compounds on mineral surfaces. Conventional decomposition-only models failed to simultaneously reproduce observed SOC stock declines and $\Delta^{14}$C dynamics without an additional non-respiratory loss pool. Although this additional C export represented a minor fraction ($< 0.5\%$) of annual decomposition flux, its cumulative effect over decadal timescales was critical for closing the mass balance. Together, these findings demonstrate that persistent SOM dynamics are governed by a combination of selective stabilization, transfer efficiency, and non-respiratory loss processes, emphasizing the need for next-generation SOM models to explicitly represent mineral-associated N controls alongside physical C transport.
\end{abstract}
...
All soil samples were dried to constant weight at 35\,$^\circ\text{C}$. The dry soil samples were homogenized and sieved ($< 2$\,mm) and roots were removed.
...
Age distributions of SOM were derived by combining age density functions with pool-specific SOC stocks. The SOC stocks of the fast, intermediate, and slow pools were extracted from the final simulation time-step of the best-fit parameterisation of the fitted multi-pool decomposition model. The probability density functions of $\text{C}_{\text{age}}$ for each pool were scaled by the corresponding final C stock to obtain stock-weighted C age distributions.
Nitrogen age distributions were reconstructed from the corresponding C age distributions using the observed final C:N ratio of each modelled pool. Specifically, the C stock density at each age was divided by the final observed C:N ratio of the respective pool to obtain the corresponding N stock density. This approach assumes that N follows the same age distribution as C within each pool and that the C:N ratio remains constant across the age distribution of a given pool. The reconstructed N distributions of the fast, intermediate, and slow pools were then summed to obtain the system-level N age distribution. Similarly, the system-level C age distribution was obtained by summing the stock-weighted C distributions of the three pools.
  
Uncertainty in the age-weighted stock distributions was quantified using the full ensemble of accepted MCMC parameter sets. For each parameter set, the final simulated C stock of each pool was used to scale its corresponding age density function, while N stock densities were reconstructed using the same pool-specific observed final C:N ratios. This produced an ensemble of stock-weighted age distributions for both C and N. Empirical 95\% uncertainty intervals were calculated across the ensemble for each age and pool or system distribution.
The median age of each distribution was calculated as the age at which the cumulative stock distribution reached 50\% of the total stock. Mean C and N ages were calculated as the first moment of the respective stock-weighted age distributions, i.e. the weighted mean of age using the age-specific stock densities as weights.
...
The soil TOC and TON contents varied among the four sites, and they decreased at all four sites over time (Fig. \ref{figure:fig2}). From the first to the last measurement, the TOC content decreased by 35\%, 37\%, and 26\% in the experiments M2, M4 and M6, and the TOC decreased by 25\% in experiment R96 since 1971.

Despite differences in soil C and N across sites, similar trends in C:N ratios justifies our approach of modeling the sites together. The fractions thermostable at 325\,$^\circ\text{C}$ and 400\,$^\circ\text{C}$ contained a larger percentage of the soil TON than of the soil TOC. Hence, the TOC:TON ratios of the thermostable fractions were low. Specifically, the median TOC:TON ratios in the fraction thermostable at 325\,$^\circ\text{C}$ at the four sites across all sampling times were 5.8, 6.9, 7.4, and 5.8, while the corresponding median C:N ratios in the fraction thermostable at 400\,$^\circ\text{C}$ were 4.3, 5.5, 5.2, and 4.1 (Fig. \ref{figure:fig2}).

Thermal fractionation generally isolated SOM pools with progressively lower $\Delta^{14}\text{C}$ values, indicating increasing age with increasing thermal stability (Fig. \ref{figure:fig2}). Across all sites, mean $\Delta^{14}\text{C}$ values declined systematically from bulk soil to the 325\,$^\circ\text{C}$ fractions (shifts ranging from $-13$\,\permil{} to $-181$\,\permil{} across sites) and from the 325\,$^\circ\text{C}$ to the 400\,$^\circ\text{C}$ fractions (shifts ranging from $-23$\,\permil{} to $-215$\,\permil{} across sites). This common pattern suggests that thermal fractionation effectively separated SOM into fractions with progressively greater ages across contrasting long-term experimental sites.
...
The turnover rate of the intermediate pool ($0.00687\,\text{y}^{-1}$) was more similar to that of the slow-cycling pool ($0.008\,\text{y}^{-1}$) than the fast-cycling pool ($0.05\,\text{y}^{-1}$) (Table \ref{table:pars}). Transfer fractions from the fast to the intermediate and from the intermediate to the slow pool, were similar ($0.128$). However, since the effective stabilization rate is the decomposition rate multiplied by the transfer coefficient, the effective flux equates to less than 1\% of the intermediate pool per year.

\subsubsection{Model diagnostics}
Trace plots showed a positive correlation between $k_i$ (intermediate pool decomposition rate) and $\alpha_{21}$ (transfer from fast to intermediate pool), ($p = 0.88$) and a negative correlation between $k_s$ (slow pool decomposition rate) and $\alpha_{21}$ ($p = -0.51$) (Fig. \ref{figure:A1}). Convergence plots confirmed equifinality of most parameters, but less so for $k_i$, $k_s$ and $\alpha_{21}$ (Fig. \ref{figure:A2}).

Although the posterior distributions for the intermediate-pool radiocarbon adjustment parameter and $\alpha_{42}$ (transfer from intermediate to loss pool) are broad (Fig. \ref{figure:A1}), their plausible ranges remain relatively narrow, suggesting that both parameters are well constrained. In contrast, the multimodal distribution of $k_i$ indicates some structural non-identifiability.

Confidence intervals (CIs) of predicted SOC stocks and $\Delta^{14}\text{C}$ values were much smaller than the standard deviation of observed measurements (Table \ref{table:preds} and Fig. \ref{figure:fig3}), supporting the value of models that incorporate data from different sites to reveal overall trends.

\subsubsection{Soil organic matter age and transit time}
Slow SOM pools exhibited broad skewed age distributions extending across centennial timescales (Fig. \ref{figure:A3}). Narrow uncertainty envelopes for fast pool ages and system transit time reflect strong constraint of rapid cycling processes by the observational data, whereas broader distributions for intermediate and slow pools indicate greater uncertainty in longer-term carbon persistence (Fig. \ref{figure:A3}).

The fast-cycling pool had a mean age of 19 years. The cycling rate of the intermediate pool was closer to that of the slow pool than the fast pool. The mean system C age (79 years) was skewed right relative to the median system age (35 years). Mean system ages of the fast and intermediate pools were also skewed right relative to their corresponding median ages, while the mean and median ages of the slow-cycling pool were relatively similar.

\subsubsection{Weighted age distributions of soil organic carbon and nitrogen}
C:N ratios decreased systematically across the age continuum, declining rapidly at younger SOM ages and approaching lower values at centennial timescales (Fig. \ref{figure:fig4}; linear regression of C against $\log_{10}$-transformed age: $\beta = -6.42$, $R^2 = 0.82$). C:N ratios differed significantly among operational SOM pools (Friedman test, $\chi^2_{(2)} = 6$, $p = 0.04979$). Fast-cycling pools had substantially higher C:N ratios (mean 22) than intermediate (6.7) and slow pools (4.6). Within each operational pool, C and N therefore had identical age distributions because N age distributions were reconstructed from the corresponding C age distributions using the observed pool-specific C:N ratios. However, because the older intermediate and slow pools had lower C:N ratios, they contained proportionally more N relative to C. Consequently, the system-level N age distribution was shifted towards older ages relative to the C distribution, resulting in different system-level mean and median ages for N and C (Fig. \ref{figure:fig5}). Thus, although C and N shared the same age distribution within individual pools, their age distributions differed when the pools were combined at the whole-system level (Fig. \ref{figure:fig5}).

\subsubsection{Carbon pool fluxes}
Fast-cycling SOM dynamics dominated the total SOM flux (Fig \ref{figure:fig6}), with $72.8\,\text{g C m}^{-2}\,\text{y}^{-1}$ respired as $\text{CO}_2$ (Fig. \ref{figure:A4}). The majority of decomposed carbon was returned to the atmosphere through respiration from all SOM pools, with median respiratory fractions of decomposition of 83\% and 91\% for the fast and intermediate pools, respectively (Table \ref{table:fluxes}). A relatively large proportion (16.7\%) of decomposed fast-pool C was partitioned into intermediate SOM, while only a small proportion (8.0\%) of intermediate C was partitioned into the slow pool. Additional export represented an almost negligible fraction ($< 0.5\%$) of decomposition flux.
...
From an agroecosystem perspective, our results suggest that C and N can exhibit different patterns of persistence even when bulk soil C:N ratios remain relatively stable over time. This decoupling in C and N persistence does not necessarily imply large changes in bulk soil C, but rather reflects differences in how C and N are distributed among SOM pools with contrasting turnover times. Thus, assessments of long-term C and N retention based solely on bulk C ratios may overlook differences in the age and persistence of C and N within the SOM pool.

Future studies should investigate the mechanisms underlying the contrasting contributions of C and N to SOM pools with different turnover times observed in this study. In particular, the lower C:N ratios of the older, more persistent pools and the resulting older system-level N age distribution suggest that N may be preferentially retained in persistent SOM pools relative to C. However, our approach does not identify the mechanisms responsible for this pattern. Differences among soil types in mineral composition and chemical conditions could influence the strength and reversibility of organo-mineral interactions and thereby affect the retention and persistence of N relative to C. For example, short-range-order Fe and Al oxides possess high surface charge density and strong affinity for organic ligands \citep{Wang2013}, potentially favouring the retention of N-containing organic compounds. Conversely, processes that alter mineral–organic interactions, such as rhizosphere acidification or competition with phosphate for sorption sites, may promote desorption of mineral-associated SOM \citep{Keiluweit2015, Spohn2022} and increase the accessibility of previously protected organic matter to microbial decomposition. Investigating these mechanisms across contrasting soils would help determine whether the greater representation of N in older SOM pools observed here reflects systematic differences in mineral protection and turnover, and would provide a mechanistic basis for understanding C-N decoupling in long-term SOM dynamics.

\conclusions %% \conclusions[modified heading if necessary]
Combining thermal fractionation, $\Delta^{14}\text{C}$ tracking, and compartmental modeling demonstrated that soil organic C and N possess distinct age distributions across the studied cropland soils. More thermostable, longer-persisting SOM fractions were systematically enriched in N compared to less stable SOM fractions, resulting in lower C:N ratios at older age distributions compared to bulk soil. This divergence indicates that selective stabilization, likely via mineral surface binding of N-rich compounds like proteins and microbial necromass, decouples long-term organic N retention from bulk C turnover. Our modeling further revealed that microbial decomposition kinetics alone could not simultaneously reproduce observed long-term SOC stock declines and $\Delta^{14}\text{C}$ dynamics. Reconciling these trajectories required incorporating an additional non-respiratory loss pool. Although this additional C export represented a small fraction of the annual decomposition flux ($< 0.5\%$), its cumulative effect over decadal timescales significantly impacted mass balances and prevent overestimating the residence time of persistent SOM pools. Standard 1D point-scale models that ignore non-respiratory export pathways risk misattributing physical C losses to heterotrophic respiration. Together, these findings highlight fundamental structural limitations in conventional first-order SOM models that assume uniform C and N turnover or ignore physical transport. To improve predictions of agricultural soil fertility and carbon cycling, next-generation Earth system and agroecosystem models must explicitly represent mineralogical controls on selective N stabilization alongside physical C export processes such as erosion and DOC leaching.

\dataavailability{Data and code associated with this study are available online \citep{Nel2026}.}

\authorcontribution{T.N. conducted formal analyses, modelling and data visualization and prepared the original manuscript draft. C.A.S. facilitated the radiocarbon analyses, developed modelling packages, assisted in research investigation and contributed to manuscript review and editing. M.S. conceptualized the study, was responsible for funding acquisition and project administration, contributed to data generation and visualization, provided supervision and contributed to manuscript writing, review and editintg.}

\competinginterests{The authors declare that they have no conflict of interest.}

\begin{acknowledgements}
The authors thank everyone who contributed to maintaining the field trials over the last decades. In addition, the authors thank Hui Liu and Anna Oskar for the laboratory work at SLU, and Axel Steinhof for the radiocarbon measurements. M.S. thanks the European Research Council for funding (101043387).
